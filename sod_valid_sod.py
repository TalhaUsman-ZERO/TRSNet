...
def main():
    ...
    pred_root = os.path.join('Prediction/'+method+'/', dataset)
    ...
    for gt_name in tqdm(gt_name_list, total=len(gt_name_list)):
        ...
        if gt.shape != pred.shape:
            cv2.imwrite( os.path.join('Prediction/'+method+'/'+dataset+'/', gt_name), cv2.resize(pred, gt.shape[::-1]))
        pred = cv2.imread(pred_path, cv2.IMREAD_GRAYSCALE)
    ...
    txt = 'Prediction/Train_record.txt'
    f = open(txt, 'a')
    f.write(eval_record)
    f.write("\n")
    f.close()
...
if __name__ == '__main__':
    ...
    parser.add_argument("--method", default='TRSNet')
